    ...
    try {
      ...
      if (propertyId && b.update_property) {
        ...
        if (cur) {
          ...
          db.prepare('UPDATE properties SET наемател=?, email=?, телефон=?, абонат_ток=?, абонат_вода=?, абонат_тец=? WHERE id=?')
            .run(upd.наемател, upd.email, upd.телефон, upd.абонат_ток, upd.абонат_вода, upd.абонат_тец, propertyId);
        }
      }
